...
const config = {
  ...
  heroVisuals: {
    ...
    colors: {
      target: { h: 205, s: 55, l: 52 }, // Muted blue for motion accents
      // The orb drifts subtly through desaturated blue tones (ambient depth, not neon).
      colorStops: [
        { h: 205, s: 48, l: 46 }, // Muted blue
        { h: 200, s: 44, l: 44 }, // Muted cyan-blue
        { h: 210, s: 42, l: 46 }, // Muted azure
        { h: 205, s: 46, l: 45 }, // Muted blue
      ],
...
function initializeHeroVisuals() {
  ...
  function animate() {
    ...
    const pullIntensity = Math.min(
      Math.hypot(virtualMouseX - centerX, virtualMouseY - centerY) /
        (rect.width / 3),
      1,
    );
    ...
    const generatedPoints = Array.from({ length: points }, (_, i) => {
      ...
      const noiseFactor =
        1 + dynamicNoiseAmount * Math.sin(time + angle * noiseFrequency);
      const stretch =
        (pullIntensity *
          dynamicMaxStretch *
          (Math.cos(angle - mouseAngle) + 1)) /
        2;
  ...
  window.addEventListener('resize', () => {
    ...
    mouseX = virtualMouseX = centerX;
    mouseY = virtualMouseY = centerY;
